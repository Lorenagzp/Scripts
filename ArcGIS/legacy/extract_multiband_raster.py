...
# Description:Gets zonal statistics from multiple bands of a Raster stores them in a feature (can extract from one feature and store statistics in another if they have the same ID or "Name" field). Gets band names from *.hdr file (needed). Can save the resulting table of the statistics
