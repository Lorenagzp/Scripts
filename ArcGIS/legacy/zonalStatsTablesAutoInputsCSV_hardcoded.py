...
################################# means -> Hardcoded thigs to watch if want to adapt to ther trial WARNING!!
...
try:      
    ...
    ## START:INPUTS ##
    ##Hard coded this
    #csvExtractThis = "C:\\Dropbox\\data\\AD\\AD_BW\\pre_data\\bw_dates_areas_bw_EYT_bwh.csv" ##########################
    csvExtractThis = "C:\\Dropbox\\data\\AE\\seed\\AE vuelos seed extract t.csv" ##########################
    #imgWs="G:\\AD15_16\\" #imageBaseLocation
    imgWs="C:\\Dropbox\\data\\AE\\seed\\mosaics\\" #test ###################################
    ##GDB whare the feature classes are stored and where the tables will be written:
    #gdb = "C:\\Dropbox\\data\\AD\\AD_BW\\shp\\ad_bw.gdb\\" #BW
    #gdb = "C:\\Users\\usuario\\Documents\\ArcGIS\\Default.gdb\\" #Default
    gdb = "C:\\Dropbox\\data\\AE\\seed\\ae_seed.gdb\\"
    ##tws = where to save tables inside the workspace
    #tws ="C:\\Dropbox\\data\\AD\\AD_BW\\shp\\pre_data\\extract\\" For BW ######Not worinkg... it save in shp folder#######
    tws ="C:\\Dropbox\\data\\AE\\seed\\ae_seed.gdb\\" #For AF ######Not worinkg... it save in shp folder#######
    #ds="extract" #Use this fro m and t ###############################
    ...
    #id_field = "id" #####################
    id_field = "Name" #####################
    log= "C:\\Dropbox\\data\\AE\\seed\\sed__ae__log.txt" ############################
    ## END:INPUTS ##
        ...
        for t,trial in enumerate(flightdate): #skip the date column
            for cam in trial.split():
                ...
                #Qué hacer si está vacío?    
                    ...
                    pattern = str(trials[t])+"("+str(dates[f])+'|__ae__)'+buf+'['+cam+']*$'
                    ...
                    if plotsF:
                        ...
                        imgLocation = imgWs##+'\\'+str(dates[f])+'\\'+cam+'\\'+sx+'\\' ######################################
                        ...
                        if exists(img):
                            ...
                            arcpy.AddMessage("|||No raster to use ||| Lookedfor: "+img)
