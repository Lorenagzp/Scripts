...
#Automatically calls the appropriate raster and zonal data from the default locations.
...
try:      
    ...
    ################################# START:INPUTS ##########################################
    #Hard coded this
    csvExtractThis = "C:\\Dropbox\\data\\AE\\bw\\ae_bw_extraction_hyper_heat.csv"
    #imgWs="G:\\AD15_16\\" #imageBaseLocation
    imgWs="G:\\AE\\" #test
    #GDB whare the feature classes are stored and where the tables will be written:
    gdb = "C:\\Dropbox\\data\\AE\\\\bw\\ae_bw.gdb\\" #BW
    #gdb = "C:\\Users\\usuario\\Documents\\ArcGIS\\Default.gdb\\" #Default
    #tws = where to save tables inside the workspace
    tws ="C:\\Dropbox\\data\\AE\\bw\\predata\\tables"######Not worinkg... it save in shp folder#######
    ds="h" #Use this ds="" fro m and t
    #ds="h" #Use this for hyper
    arcpy.env.workspace = gdb
    #arcpy.AddMessage("wspace: "+str(arcpy.env.workspace))
    #knownBands="62" # ##################### hyper resampled
    knownBands="62" # ####################### thermal
    id_field = "Name" #Table field that has the unique plot identifier
    log= "C:\\Dropbox\\data\\AE\\bw\\log.txt"
    ## END:INPUTS ##
    ...
    arcpy.env.workspace = gdb
    ext=".bsq" #extension of the rasters
    buf="buf" #we use the buffered plot features -buf- to extract the stats or the full plots -plt-
    bw8="xxx" #default trial eg. bw8 if there is a raster that includes all the zonal features
        ...
        for t,trial in enumerate(flightdate): #skip the date column
            for cam in trial.split():
                ...
                #Qué hacer si está vacío?    
                    ...
                    pattern = str(trials[t])+"("+str(dates[f])+'|__ad__)'+buf+'['+cam+']*$'##################################################include this ifthere are different grids by camera
                    ...
                    if plotsF:
                        ...
                        mF, sx = mosaicFolder.get(cam,("geo","geo")) #corresponding folder and sfx, geo default
                        imgLocation = imgWs+str(dates[f])+"\\"+cam+"\\"+mF+"\\"
