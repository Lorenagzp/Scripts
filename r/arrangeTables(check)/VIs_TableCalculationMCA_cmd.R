...
#It can accept multiple input files to execute the tool on each of them
#####################################################################
...
for (arg in 1:length(args)) {
  ...
  rfl_data <- read.xlsx2(args[arg], sheetIndex = 1,colClasses = c('character',rep('numeric',6)), stringsAsFactors=FALSE)
  print("Data read succesfully")
  colnames(rfl_data) <- c("Plot",bandWl)
  rfl_data <- divideMCA6Table(rfl_data,100000) # Make the range from 0-1 (For The MCA I need to divide by 100000)
  ### Calculate the VIS! (append them to the table)
  rfl_data_VI <- VI_MCA_tab(rfl_data)
