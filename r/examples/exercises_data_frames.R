...
##library(corrplot), plotear matrix de correlaci�n
corrgram(cor)
corrplot(cor, type = "upper")
corrplot.mixed(cor)

library(corrplot) #tables

## with this we could merge the tables, but it is not what we need exactly 
#tables <- merge(t_h,t_b,by="Plot")
#tables <- merge(tables,t_m,by="Plot")

############### REad write kml
## thanks  to http://spatial.ly/2011/01/exporting-kml-from-r/
library(maptools)
require(rgdal)
foo = readOGR("SOLOBLOQUES.kml", "SOLOBLOQUES") # el segundo es el id del kml
...
writeOGR(foo, dsn="BloquesValleYaqui.kml", layer= "foo", driver="KML", dataset_options=c("NameField=Max_Bloque"))


##### Woring with filepaths and filenames
path<-dirname(normalizePath(args[1]))
#print(path)
#Img basename without extention
name<-gsub(".tif", "", args[1])
