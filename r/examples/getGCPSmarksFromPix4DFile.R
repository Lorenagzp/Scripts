...
for (f in p4dFiles) {
  ...
  ## If you need the image names and coordinates list
  #imgs_name<-  xpathApply(imgs,"//image/@path") # Get the list of image names
  #imgs_coords<- xpathApply(imgs,"//image//gps") # Get the list of image coords
  ...
  saveXML(gcps, file.path(outpath, paste0(outfilename,"_gcps.XML")), fsep=.Platform$file.sep) #Save gcps info
}